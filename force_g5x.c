		...
		double eps, double (*a)[3], double *p)
{
    ...
    g5x_set_jp(0, nj, mj, xj);
    g5x_set_n(nj);
    g5x_set_eps2_to_all(eps*eps);
	...
	/* send i particle */
	g5x_set_xi(nii, (double (*)[3])xi[i]);

	/* run and get force */
	g5x_get_force(nii, (double (*)[3])a[i], p+i);

    }
    if (firstcall) {
	firstcall = 0;
	fprintf(stderr, "Warning: G5X does not calculate potential. The value returned is just a dummy.\n");
    }
    for (i = 0; i < ni; i++) {
	p[i] = -0.1;
    }
		...
		double eps, double (*a)[3], double *p)
{
	...
	for (i = 0; i < ni; i += NIPMAX) {
	    ...
	    /* send i particle */
	    // g5x_set_xi(nii, (double (*)[3])xi[i]);
	    g5x_set_xi_and_run(nii, (double (*)[3])xi[i]);

	    /* run and get force */
	    // g5x_get_force(nii, (double (*)[3])atmp[i], p+i);
	    g5x_get_force_without_run(nii, (double (*)[3])atmp[i], p+i);
	}
	for (i = 0; i < ni; i++) {
	    for (k = 0; k < 3; k++) {
		a[i][k] += atmp[i][k];
