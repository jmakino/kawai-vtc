...
vtc_set_scale(double xmax, double mmin)
{
    ...
    g5e_set_range(-xmax, xmax, mmin);
}

#if 0 /* the most simple one */
		...
		double eps, double (*a)[3], double *p)
{
    ...
    if (JMEMSIZE < nj) {
	fprintf(stderr, "nj: %d exceeded GRAPE-5X JMEMSIZE (%d)\n", nj, JMEMSIZE);
	exit(1);
    }
    if (!grape_is_opened()) {
	grape_open();
    }
    g5e_set_jp(0, nj, mj, xj);
    g5e_set_n(nj);
    g5e_set_eps2_to_all(eps*eps);
    for (i = 0; i < ni; i += NPIPES) {
	int nii;
	...
	/* send i particle */
	g5e_set_xi(nii, (double (*)[3])xi[i]);

	/* run and get force */
	g5e_get_force(nii, (double (*)[3])a[i], p+i);

    }
    if (firstcall) {
	firstcall = 0;
	fprintf(stderr, "Warning: G5E does not calculate potential. The value returned is just a dummy.\n");
    }
    for (i = 0; i < ni; i++) {
	p[i] = -0.1;
    }
...
#elif 1 /* can handle nj larger than JMEMSIZE */
		...
		double eps, double (*a)[3], double *p)
{
	...
	// !!!
	// g5e_set_jp_nomjconv(0, njj, mj + j, xj + j);
	g5e_set_jp(0, njj, mj + j, xj + j);
	g5e_set_n(njj);
	g5e_set_eps2_to_all(eps*eps);
	for (i = 0; i < ni; i += NIPMAX) {
	    int nii;
	    ...
	    /* send i particle */
	    // g5e_set_xi(nii, (double (*)[3])xi[i]);
	    g5e_set_xi_and_run(nii, (double (*)[3])xi[i]);

	    /* run and get force */
	    // g5e_get_force(nii, (double (*)[3])atmp[i], p+i);
	    g5e_get_force_without_run(nii, (double (*)[3])atmp[i], p+i);
	}
	for (i = 0; i < ni; i++) {
	    for (k = 0; k < 3; k++) {
		a[i][k] += atmp[i][k];
